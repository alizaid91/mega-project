...
const WeatherDisplay = ({ weatherData, display, onClose }) => {
    ...
    return (
        <div className={`${display ? 'visible' : 'invisible'} z-50 fixed w-screen h-screen top-0 left-0 flex bg-black/60 backdrop-blur-md items-center justify-center`}>
            <div className={`${display ? 'opacity-100 scale-100' : 'scale-110 opacity-0'} relative mx-2 md:mx-4 transition-all duration-500 bg-white rounded-xl shadow-md px-3 md:px-6 py-8`}>
                <div className="pl-3 pr-1 py-1 flex justify-between rounded-xl items-center bg-black/20 mb-3">
                    <div className='pr-3'>
